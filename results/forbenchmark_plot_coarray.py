...
def read_benchmark_data(directory):
    ...
    num_arguments = len(data_rows[0]) - 9  # Assuming the first 9 columns are fixed

    # Define column names based on the inferred number of arguments
    columns = ['method', 'speedup_max', 'elapsed_time_image', 'elapsed_time_max', 'elapsed_time_min', 'elapsed_time_avg', 'gflops_image', 'gflops_tot', 'nloops'] + [f'arg{i}' for i in range(1, num_arguments + 1)]
    ...
    arg_data = benchmark_data.iloc[:, 9:]
...
def plot_benchmark_data(file_path, benchmark_data, x_data, title, xlabel, ylabel, data_column):
    ...
    if data_column == 'elapsed_time_avg':
        output_filename = f'{os.path.splitext(file_path)[0]}_time_avg.png' # TODO: remove image number from file the file name
    if data_column == 'elapsed_time_max':
        output_filename = f'{os.path.splitext(file_path)[0]}_time_max.png' # TODO: remove image number from file the file name
    elif data_column == 'gflops_tot':
        output_filename = f'{os.path.splitext(file_path)[0]}_perf_tot.png' # TODO: remove image number from file the file name
    elif data_column == 'elapsed_time_image':
        output_filename = f'{os.path.splitext(file_path)[0]}_time_image.png'
    elif data_column == 'gflops_image':
        output_filename = f'{os.path.splitext(file_path)[0]}_perf_image.png'
    elif data_column == 'speedup_max':
        output_filename = f'{os.path.splitext(file_path)[0]}_speedup_max.png' # TODO: remove image number from file the file name
