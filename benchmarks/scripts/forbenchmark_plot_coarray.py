...
def read_benchmark_data(directory):
    ...
    if len(sys.argv) != 2:
        print("Usage: python plot_co.py results/<file_name>")
        sys.exit(1)
...
def plot_benchmark_speedup_max(file_path, benchmark_data, x_data, title, xlabel, ylabel, bar_width):
    ...
    for i, (method, group) in enumerate(benchmark_data.groupby("method")):
        speedup_values = group["speedup_max"]
    ...
    plt.yticks(np.arange(0, round(max(benchmark_data["speedup_max"]))+2, 1))
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    # plt.legend()
...
def plot_average_speedup_max(file_path, benchmark_data, x_data, title, xlabel, ylabel):
    ...
    weighted_speedup = benchmark_data['speedup_max'] * benchmark_data['weights']
    ...
    output_filename = f"{os.path.splitext(file_path)[0]}_speedup_max_avg.png"
    plt.savefig(output_filename, dpi=DPI)
...
def plot_speedup_max(file_path, benchmark_data, x_data, title, xlabel, ylabel, bar_width):
    """
    Plot performance (GFLOPS) data from benchmark data.
    ...
    Returns:
        plt (matplotlib.pyplot): Matplotlib pyplot object for the generated plot
    """
    return plot_benchmark_speedup_max(
        file_path, benchmark_data, x_data, title, xlabel, ylabel, bar_width
    )


def plot_speedup_max_avg(file_path, benchmark_data, x_data, title, xlabel, ylabel):
    """
    Plot performance (GFLOPS) data from benchmark data.
    ...
    Returns:
        plt (matplotlib.pyplot): Matplotlib pyplot object for the generated plot
    """
    return plot_average_speedup_max(
